...
const availableItems: Item[] = [
  {
    name: "Controller 🕹️",
    description:
      "Standard controller for mashing purposes. The buttons are already worn down.",
    cost: 10,
    rate: 0.1,
    count: 0,
  },
  {
    name: "Button Mashing Grandma 👵",
    description:
      "Poached from the cookie farm layoffs, worse than you at mashing.",
    cost: 100,
    rate: 2,
    count: 0,
  },
  {
    name: "Turbo Controller ⏩",
    description:
      "Makes your grandmas more efficient by replacing mashing buttons with holding them.",
    cost: 1000,
    rate: 50,
    count: 0,
  },
  {
    name: "Button Mashing Machine 🤖",
    description:
      "Much faster than you or your grandmas at mashing those buttons.",
    cost: 2000,
    rate: 100,
    count: 0,
  },
  {
    name: "Ludwig 👨",
    description: "Self-proclaimed best button masher in the world.",
    cost: 10000,
    rate: 1000,
    count: 0,
  },
...
function upgradeRate(i: number, _event: Event) {
  ...
  const upgradeButton = document.getElementById(
    `Buy ${availableItems[index].name}`,
  ) as HTMLButtonElement;
  const upgradeButtonScreen = document.getElementById(
    `${availableItems[index].name}`,
  );
  if (upgradeButtonScreen)
    upgradeButtonScreen.innerHTML = `${availableItems[index].count} ${availableItems[index].name} owned - ${availableItems[index].description}`;
...
const resourceValueCounter = document.createElement("h2");
...
availableItems.forEach((item, index) => {
  ...
  upgradeButton.style.cssText =
    "color: white; background-color: black; text-decoration: line-through;";
  upgradeButton.disabled = true;
  app.append(upgradeCountScreen, upgradeButton);

  upgradeButton.addEventListener(
    "click",
    (event) => upgradeRate(index, event),
    false,
  );
});
